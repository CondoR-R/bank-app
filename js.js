...
const account1 = {
  ...
  movementsDates: [
    ...
    "2024-10-22T05:01:17.194Z",
    "2024-10-23T05:36:17.929Z",
...
function formatMovementDate(date) {
  const culcDaysPassed = (date1, date2) =>
    Math.round((date1 - date2) / (1000 * 60 * 60 * 24));
  const daysPassed = culcDaysPassed(new Date(), date);
  console.log(daysPassed);

  if (daysPassed === 0) return "Сегодня";
  if (daysPassed === 1) return "Вчера";
  if (daysPassed <= 7)
    return `Прошло ${daysPassed} ${daysPassed < 5 ? "дня" : "дней"}`;
...
function calcDisplaySum(acc) {
  ...
  labelSumIn.textContent = Intl.NumberFormat(acc.locale, valuteOptions).format(
    incomes
  );
  ...
  const out = acc.movements
    .filter((mov) => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  labelSumOut.textContent = Intl.NumberFormat(acc.locale, valuteOptions).format(
    Math.abs(out)
  );
...
btnLogin.addEventListener("click", function (e) {
  ...
  if (currentAccount && currentAccount.pin === Number(inputLoginPin.value)) {
    ...
    // если таймер есть, то сбрасываем его и начинаем заново (при смене акка)
